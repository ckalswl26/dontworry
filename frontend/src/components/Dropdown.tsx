...
export function Dropdown({
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <span className={`min-w-0 truncate ${selected ? "font-medium text-brand-navy" : "text-slate-400"}`}>
      ...
      {open && (
        <div className={`absolute left-0 right-0 z-40 max-h-56 overflow-x-hidden overflow-y-auto overscroll-contain rounded-xl border border-slate-100 bg-white p-1.5 shadow-[0_12px_28px_rgba(17,28,78,0.16)] ${placement === "up" ? "bottom-full mb-1.5" : "top-full mt-1.5"}`}>
          {options.map((opt) => (
            <button
              key={opt.value}
              type="button"
